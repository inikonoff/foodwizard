import asyncio # <--- Исправленный импорт
import os
import logging
import sys
import contextlib
...
# Твои модули (Проверь, чтобы все эти импорты были верны в твоем проекте)
...
# --- НАСТРОЙКА МЕНЮ (Заглушка) ---
async def setup_bot_commands(bot: Bot):
    """Устанавливает команды меню для всех языков"""
    # Вставь здесь свой код setup_bot_commands из common.py
    commands = [
    ...
    try:
        ...
        logger.error(f"Не удалось установить команды: {e}", exc_info=True)
    ...
    async with lifespan():
        ...
        try:
            ...
            logger.info("✅ Старые Polling/Webhook сессии сброшены.")
             ...
             logger.warning(f"Ошибка сброса Webhook: {e}", exc_info=True)
