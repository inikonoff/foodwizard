    ...
    for trigger in triggers:
        if clean_text.startswith(trigger):
            ...
            if dish_name and len(dish_name) > 1:
                # !!! ИСПРАВЛЕНИЕ ТУТ: strip(), удаление знаков в конце, и TITLE CASE !!!
                clean_name = dish_name.strip().rstrip('.?!')
                return clean_name.title() 
                # .title() делает "Apple Pie", "Pizza Margherita"
    return None
...
async def handle_back_to_categories(callback: CallbackQuery):
    ...
    for category in categories:
        clean_key = category.lower().strip()
        label = get_text(lang, clean_key)
        if not label or label == clean_key: label = category.title()
        builder.row(InlineKeyboardButton(text=label, callback_data=f"cat_{clean_key}"))
