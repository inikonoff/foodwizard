...
async def handle_dish_selection(callback: CallbackQuery):
    ...
    user_data = await users_repo.get_user(user_id)
    lang = user_data.get('language_code', 'ru')
    is_premium = user_data.get('is_premium', False) # <-- ПРЕМИУМ ФЛАГ
    ...
    try:
        ...
        # Генерация с учетом премиума (КБЖУ)
        recipe = await groq_service.generate_recipe(dish.get('name'), products, lang, user_id, is_premium)
        await wait_msg.delete()
        ...
        await track_safely(user_id, "recipe_generated", {"dish": dish.get('name')})
        ...
        logger.error(f"Error dish: {e}", exc_info=True)
        await callback.message.answer(get_text(lang, "error_generation"))
...
async def handle_back_to_categories(callback: CallbackQuery):
    ...
    for category in categories:
        builder.row(InlineKeyboardButton(text=get_text(lang, category), callback_data=f"cat_{category}"))
    builder.row(InlineKeyboardButton(text=get_text(lang, "btn_restart"), callback_data="restart"))
    await callback.message.edit_text(get_text(lang, "choose_category"), reply_markup=builder.as_markup())
