...
def safe_format_recipe_text(text: str) -> str:
    """
    Превращает Markdown от LLM в красивый и безопасный HTML для Telegram.
    """
    if not text:
        return ""

    # 1. Сначала экранируем критические символы (<, >, &), 
    # чтобы текст самого рецепта не сломал HTML-разметку.
    text = html.quote(text)

    # 2. Заголовки (### Заголовок -> <b>Заголовок</b>)
    # Groq любит использовать ### для названий разделов
    text = re.sub(r'#{1,6}\s*(.*?)$', r'<b>\1</b>', text, flags=re.MULTILINE)

    # 3. Жирный текст (**текст** -> <b>текст</b>)
    text = re.sub(r'\*\*(.*?)\*\*', r'<b>\1</b>', text)
    
    # 4. Альтернативный жирный (__текст__ -> <b>текст</b>)
    text = re.sub(r'__(.*?)__', r'<b>\1</b>', text)

    # 5. Курсив (*текст* -> <i>текст</i>)
    # Важно делать это ПОСЛЕ жирного, чтобы не сломать **
    text = re.sub(r'(?<!\*)\*(?!\*)(.*?)(?<!\*)\*(?!\*)', r'<i>\1</i>', text)

    # 6. Списки (- пункт -> • пункт)
    # Делаем списки красивыми буллитами
